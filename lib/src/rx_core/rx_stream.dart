...
class RxStream<T> {
  ...
  RxStream({this.onListen, this.onPause, this.onResume, this.onCancel});
  ...
  Stream<T> get stream =>
      RxStreamTransformation(addSubscription, removeSubscription);
}
...
class RxStreamTransformation<T> extends Stream<T> {
  final AddSubscription<T> _addSubscription;
  final RemoveSubscription<T> _removeSubscription;
  RxStreamTransformation(this._addSubscription, this._removeSubscription);
